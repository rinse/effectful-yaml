...
- したがって欠落は、パスアクセスの欠落と同じように [std.opt](std.opt.md) や [std.prune](std.prune.md)、`$handle` で扱える。
- `in` の値がマッピングでないことと `key` の値が文字列でないことは、データの変動ではなく形の誤りなのでエラーになる。捕捉できない。
- 実装は O(1) の照会で最適化してよいが、観測できる振る舞いは下の展開と一致する。
- `{$op: std.lookup}` で関数値として参照できる（演算であり、std の派生ハンドラではないので `$op` の対象になる）。
...
- [std.prune](std.prune.md)（失敗を分岐の打ち切りに変える）
