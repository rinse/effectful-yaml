...
- `$in` を省いて初期値だけを書いた `{$std.state: 初期値}` は、`$do` の文としても、マッピングのキーとしても置け、それを置いた並びの残りに状態を導入する。残りが本体である（[言語仕様の文脈の導入の節](../grammar.md)）。
...
貫流と分岐点の区別は、`$in` を省いた `$std.state` をどの `$do` の文に置くかでも表現できる。
`$std.list` を包む外側の `$do` に置けば貫流になり、`$std.list` の引数である内側の `$do` の先頭（選択より前）に置けば分岐点で分かれる。
...
外側の `$do` の文に置いた `$std.state` が `$std.list` を包み、状態が分岐から分岐へ持ち越される。
