...
- 初期値だけを書いた `{$std.state: 初期値}` は前置きであり、`$do` の文としても、マッピングのキーとしても置ける。本体はそれを置いた文脈の残りである（[言語仕様の前置きの節](../grammar.md)）。
...
前置きとしてマッピングのキーに置けば、状態のスコープはそのマッピングの中で閉じる。
...
- [$do](do.md)（`$std.state` の前置きを置ける文の位置）
- [$let](let.md)（同じく前置きに置ける二項形）
- [言語仕様の前置きの節](../grammar.md)
