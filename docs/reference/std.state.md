...
展開の全体は、本体を `$in` に置いてこのハンドラで包み、得られた状態変換関数を初期値のエントリの列に適用する形である。
初期値の式は本体より先に評価される。
...
初期値だけを書いた `{$std.state: 初期値}` は前置きであり、`$do` の文としても、マッピングのキーとしても置ける。
...
残りの無い `{$std.state: 初期値}` は `$do` の文の位置でだけ書ける。

「貫流」と「分岐点で分かれる」の区別は、この前置きの置き場所でも表現できる。
`$std.state` の前置きを `$std.list` を包む外側の `$do` に置けば貫流になり、`$std.list` の引数である内側の `$do` の先頭（選択より前）に置けば分岐点で分かれる。
...
外側の `$do` に置いた `$std.state` の前置きが `$std.list` を包み、状態が分岐から分岐へ持ち越される。

マッピングのキーに置けば、状態のスコープはそのマッピングの中で閉じる。
...
`$std.state` の初期値は、先に書いた `$let` の束縛を見る。
