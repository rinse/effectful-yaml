...
- 入力作用は reader モナドに、状態作用は state モナドに、選択は list モナドに対応する。選択の解釈は `std.list` などの明示のハンドラが与える。
...
1. **流れの健全性**。実行時の閉包はすべて文書内の `$fn` から作られる（入力とホストの値の呼び出しの結果はデータであり、環境は不変で `$let` は非再帰なので、閉包が自分の捕獲環境から到達されることもない）。`std` の関数は処理系が固定した有限個の関数であり、受け取った関数を有限回だけ適用する。評価前の推論は、各位置に現れうる閉包の集合を過大近似する。
