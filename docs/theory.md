...
言語仕様の「関数値の流れと停止性」の検査に通った文書が停止することは、次の三段で論証できる。

1. **流れの健全性**。実行時の閉包はすべて文書内の `$fn` から作られる（パラメータとホスト演算の結果はデータであり、環境は不変で `$let` は非再帰なので、閉包が自分の捕獲環境から到達されることもない）。評価前の推論は、各位置に現れうる閉包の集合を過大近似する。
2. **非循環なら有限の型が付く**。`$fn` の間の依存関係が循環を持たなければ、その位相順に沿って各 `$fn` に有限の単純型（選択を含む合併型）を割り当てられる。自己適用が要求する `T = T → U` の形の再帰型（μ 型）は構成から排除される。単純型だけでは足りないことに注意する。状態のセルは参照を通じた一般再帰（Landin の knot、fix の再現）を許すので、依存には演算の値の経路——状態を通って戻る関数値——も数えなければならない。
3. **強正規化**。得られる体系は、fix を持たない fine-grained call-by-value に有限型の関数と代数的作用・深いハンドラを加えたものである。ハンドラは有限の演算木の上の畳み込みであり、`$resume` を複数回呼んでも継続の中の適用とその型の関係は変わらないので、継続の適用は新たな循環を生まない（Plotkin–Pretnar の系譜の議論。検査が `$resume` の結果の適用を依存に数えないのはこのためである）。`$collect` と `std.range` は有限のデータしか回らない。

本書は規範ではないので、これは証明の設計図であって形式的な証明ではない。
