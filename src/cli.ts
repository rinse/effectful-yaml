...
const USAGE = `\
Usage: eff-yaml [file] [-i name=value]... [--ops module]... [-o file] [--check]

effectful-yaml 文書を評価し、結果を YAML で標準出力に書く。
$std.log の中身は標準エラー出力に書く。

  file             対象のファイル。省略時と "-" は標準入力。
  -i, --input      起動時の入力。値は YAML として解釈する。繰り返し可。
...
function buildHeader(file: string | undefined, input: string[], ops: string[]): string {
  const from = file === undefined || file === '-' ? '' : ` from ${file}`;
  const withInput = input.map((p) => ` -i ${p}`).join('');
  const withOps = ops.map((o) => ` --ops ${o}`).join('');
  return `# Code generated by eff-yaml${from}${withInput}${withOps}; DO NOT EDIT.\n`;
}

function buildRegenerateCommand(file: string | undefined, input: string[], ops: string[], output: string): string {
  const target = file === undefined ? '' : ` ${file}`;
  const withInput = input.map((p) => ` -i ${p}`).join('');
  const withOps = ops.map((o) => ` --ops ${o}`).join('');
  return `eff-yaml${target}${withInput}${withOps} -o ${output}`;
}
...
async function checkOutput(
  ...
  const regenCmd = buildRegenerateCommand(file, input, ops, outFile);
  let existing: Buffer;
  try {
...
function parseInput(pairs: string[]): Record<string, Value> {
  const input: Record<string, Value> = {};
  for (const pair of pairs) {
    const eq = pair.indexOf('=');
    if (eq <= 0) throw new Error(`--input には name=value の形を渡す: ${pair}`);
    input[pair.slice(0, eq)] = parse(pair.slice(eq + 1)) as Value;
  }
  return input;
}
...
export async function run(argv: string[], io: CliIo): Promise<number> {
  try {
    ...
    if (positionals.length > 1) throw new Error(`対象のファイルは一つだけ指定できる: ${positionals.join(', ')}`);
    ...
    const expected = buildHeader(file, values.input ?? [], opsPaths) + rendered;
    if (values.check) {
      return await checkOutput(values.output, expected, file, values.input ?? [], opsPaths, io);
    }
