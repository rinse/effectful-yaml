/**
 * 検証テスト：docs/grammar/（草案 0.13）が定める展開を文書として書き、同じ本体を
...
describe('std.mapping の展開との等価性', () => {
  it('grammar/examples.md の svc- 用例（std.each によるマッピングの分解と組み立て）が一致する', async () => {
    const body = `
$do:
- $let:
...
// -----------------------------------------------------------------------------
// 6. std.where（grammar/std.md「std.where」）
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// 7. fold の導出（std.collect + std.state、grammar/std.md「std.collect」）
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// 9. std.lookup の展開（grammar/std.md「std.lookup」）
// -----------------------------------------------------------------------------

/** grammar/std.md「std.lookup の値」に引数を渡した形。inFlow はフロー形式（例: "{a: 1}"）。 */
...
// -----------------------------------------------------------------------------
// 10. $do の文に置いた文脈の導入の展開（grammar/derived.md「$do」）
...
// -----------------------------------------------------------------------------
// 12. $for の展開（grammar/derived.md「$for」）
