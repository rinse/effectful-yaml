...
// -----------------------------------------------------------------------------
// 4. $std.first の展開（docs/reference/std.first.md の概形を完全にしたもの）
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// 9. $do の文に置いた前置きの展開（grammar.md「$do」）
//   {$do: [{$std.state: 初期値}, 残り...]} ≡ {$std.state: 初期値, $in: {$do: [残り...]}}
//   {$do: [{$with: 節}, 残り...]}          ≡ {$in: {$do: [残り...]}, $with: 節}
// -----------------------------------------------------------------------------

describe('$do の文に置いた前置きの展開との等価性', () => {
  it('$with の前置きは残りの文を包む $with と一致する（値もログの順序も）', async () => {
    const statement = `
$do:
- $with:
    std.fail:
...
`;
  ...
  it('$std.state の前置きは、残りの文を $in に取る二項形と一致する（初期値は文の位置で評価される）', async () => {
    const statement = `
$do:
- $let:
    base: 41
