 ...
 * 4. 文脈の導入を伴うブロック形式のマッピングは、残りがデータなら島にならない。頭の `$` の対が
...
describe('文脈の導入を伴うマッピング', () => {
  it('頭の対は行内コメントごと消え、データのキーのコメントは残る', async () => {
    const src = `# 先頭
$let:
  registry: ghcr.io/acme   # 頭の中
name: api   # 名前
# データのキーの前
image: \${registry}/api
# 末尾
...
`);
  ...
  it('頭より前に書かれたデータのキーも、コメントごとその場に残る', async () => {
...
`);
  ...
  it('残りが主形の文脈の導入は島として置換される', async () => {
...
`);
  ...
  it('フロー形式の文脈の導入は島として置換される', async () => {
...
`);
  ...
  it('シーケンスの標識と同じ行の文脈の導入は、その要素ごと島として置換される', async () => {
    const src = `services:
- $let: {port: 8080}
  name: web   # 消える
  port: \${port}
