 ...
 *
 * 拒否側は、作用の推論（Analyzer）の追跡の盲点（$collect の $with 引数、ハンドラ節の引数、
...
`),
  ...
  it('$collect の $with の引数を通る密輸も拒否される（追跡の盲点を持たない）', async () => {
    // 作用の推論は $with の引数を追跡しない（eval.ts の意図的な上限）。流れの検査は全域。
    await expect(
      run(`
$collect:
- $fn: c
...
`),
  ...
  it('ハンドラの節の引数を通る密輸も拒否される（追跡の盲点を持たない）', async () => {
    await expect(
      run(`
$handle:
  $my.op:
