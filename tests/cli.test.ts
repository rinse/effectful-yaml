...
describe('eff-yaml CLI', () => {
  it('標準入力の文書を評価して YAML を標準出力に書く', async () => {
    const r = await runCli(
      ['-i', 'db_host=example.com'],
      'server: {host: {$std.input: db_host}, port: {$std.input: db_port, $default: 5432}}',
    );
    expect(r.code).toBe(0);
    // 島は二つの $std.input だけなので、原文のフロー形式が保たれる。
  ...
  it('パラメータの値は YAML として解釈される', async () => {
    const r = await runCli(['-i', 'n=3'], 'value: {$std.input: n}');
    expect(r.code).toBe(0);
  ...
  it('name=value の形でない --input は拒否する', async () => {
    const r = await runCli(['-i', 'oops'], 'null');
    expect(r.code).toBe(1);
...
`,
      ...
      const r = await runCli(
        ['--ops', opsFile],
        '{$handler: {str.upper: {$param: s, $fn: {$resume: nope}}}, $in: {$str.upper: hello}}',
      );
      expect(r.code).toBe(1);
  ...
  describe('-o, --output', () => {
    ...
    it('ファイル入力かつ -i 複数指定でヘッダにそのまま並べる', async () => {
      dir = await mkdtemp(join(tmpdir(), 'eff-yaml-'));
      const inFile = join(dir, 'config.eyaml');
      const outFile = join(dir, 'out.yaml');
      await writeFile(inFile, 'value: {$std.input: n}', 'utf8');
      const r = await run(['-i', 'env=prod', '-i', 'n=1', '-o', outFile, inFile], {
        stdin: (async function* () {})(),
        stdout: () => {},
        stderr: () => {},
      });
      ...
      const written = await readFile(outFile, 'utf8');
      expect(written.split('\n')[0]).toBe(
        `# Code generated by eff-yaml from ${inFile} -i env=prod -i n=1; DO NOT EDIT.`,
      );
    });
  ...
  describe('--check', () => {
    ...
    it('出力ファイルが古いと 1 で、ファイル名・再生成コマンド・差分を書く', async () => {
      ...
      const outFile = join(dir, 'out.yaml');
      await writeFile(inFile, 'value: {$std.input: n}', 'utf8');
      await writeFile(outFile, `# Code generated by eff-yaml from ${inFile} -i n=1; DO NOT EDIT.\nvalue: 0\n`, 'utf8');
      let out = '';
      let err = '';
      const r = await run(['-i', 'n=1', '-o', outFile, '--check', inFile], {
        stdin: (async function* () {})(),
        stdout: (t) => (out += t),
        stderr: (t) => (err += t),
      });
      ...
      expect(err).toContain(`eff-yaml ${inFile} -i n=1 -o ${outFile}`);
